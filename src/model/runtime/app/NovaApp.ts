...
import type { NovaRaphPhaseContext, NovaRaphRuntime } from '@/model/runtime/app/NovaRaphRuntime'
import type { NovaScene } from '@/model/runtime/scene/NovaScene'
import type { NovaRaphNode } from '@/model/runtime/tree/NovaRaphNode'
import { EventBus } from '@endge/utils'
import { RaphSchedulerType } from '@raphy-js/raph/runtime'
...
import { RaphLocalPhase } from '@/model/runtime/app/nova-raph-metadata'
import { processDirtyNodes } from '@/model/runtime/app/NovaRaphRuntime'
...
export class NovaApp<E extends EventList = Record<string, any>> {
  ...
  private readonly _raph: NovaRaphRuntime<NovaNodeProperties>
  ...
  before(p: NovaRaphPhaseContext<NovaNodeProperties>): void {
  ...
  preupdate(p: NovaRaphPhaseContext<NovaNodeProperties>): void {
  ...
  update(p: NovaRaphPhaseContext<NovaNodeProperties>): void {
    this._diagnostics.recordDirtyNodes(p.dirty.length)
  ...
  matrix(p: NovaRaphPhaseContext<NovaNodeProperties>): void {
    this._diagnostics.recordDirtyNodes(p.dirty.length)
  ...
  render(p: NovaRaphPhaseContext<NovaNodeProperties>): void {
  ...
  addSurface<T extends NovaSurface<E>>(surface: T): T {
    ...
    this.raph.addNode(surface as unknown as NovaRaphNode<NovaNodeProperties>)
