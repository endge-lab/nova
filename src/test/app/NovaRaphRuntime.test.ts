...
import { RaphKernel } from '@raphy-js/raph/path'
...
describe('проверка Runtime-мост Nova и Raph', () => {
  ...
  it('разрешает переменные пути и не будит соседний data path', () => {
    const kernel = new RaphKernel()
    const app = createTestApp({ raph: { kernel } })
    const node = createAuditNode(app, app.createSurface('dynamic-path'))

    node.observeData('items[id=$id].version', { vars: { id: 2 } })
    kernel.set('items[id=1].version', 1)
    app.raph.run()
    expect(node.updates).toBe(0)

    kernel.set('items[id=2].version', 1)
    app.raph.run()
    expect(node.updates).toBe(1)
  ...
  it('не теряет повторную инвалидацию из выполняющейся фазы', () => {
    const app = createTestApp()
    const surface = app.createSurface('reentrant-update')
    class ReentrantNode extends RuntimeAuditNode {
      queueAnotherUpdate = false

      override update(): void {
        super.update()
        if (!this.queueAnotherUpdate) { return }
        this.queueAnotherUpdate = false
        this.raph.dirty('update', this)
      }
    }
    const node = new ReentrantNode(app, surface)
    surface.addChild(node)
    app.raph.run()
    node.updates = 0
    node.queueAnotherUpdate = true

    node.raph.dirty('update', node)
    app.raph.run()
    app.raph.run()
  ...
  it('сохраняет локальные свойства в немедленном пути Raph', () => {
    ...
    const writeSpy = vi.spyOn(node.raph.kernel, 'set')
  ...
  it('игнорирует отложенную инвалидацию после уничтожения приложения', () => {
    const app = createTestApp()
    const node = createAuditNode(app, app.createSurface('late-refresh'))

    app.destroy()

    expect(() => {
      app.raph.dirty('render', node)
      app.raph.invalidate()
      app.raph.run()
    }).not.toThrow()
  })
