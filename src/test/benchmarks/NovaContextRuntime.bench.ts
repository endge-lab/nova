...
describe('бенчмарки контекста и runtime Nova', () => {
  ...
  bench('local property fast path: 200k x/y writes without DataPath notify', () => {
    ...
    if (notifySpy.some(spy => spy.mock.calls.length > 0)) {
      throw new Error('Local property fast path wrote to RaphKernel')
    }
  }, benchOptions)
})
...
function createInjectFixture(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    const node = new BenchNode(app, surface)
    provider.addChild(node, { invalidate: false })
    nodes.push(node)
  }
...
function createReparentFixture(count: number) {
  ...
  sourceParent.addChild(subtreeRoot, { invalidate: false })

  for (let i = 0; i < count; i++) {
    const child = new BenchNode(app, surface)
    subtreeRoot.addChild(child, { invalidate: false })
    child.inject(token)
  }
...
function createLocalPropertyFixture() {
  ...
  const node = createBenchNode(app, surface)
  const notifySpy = [
    vi.spyOn(app.raph.kernel, 'set'),
    vi.spyOn(app.raph.kernel, 'merge'),
    vi.spyOn(app.raph.kernel, 'delete'),
  ]
